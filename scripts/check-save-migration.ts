...
assert.equal(loadedLegacy.name, 'mint-legacy');
assert.equal(loadedLegacy.level, 37);
assert.equal(loadedLegacy.coins, 4321);
assert.equal(loadedLegacy.hearts, 87);
assert.equal(loadedLegacy.inventory.emergency_biscuit, 12);
assert.equal(loadedLegacy.inventory.strawberry_cake, 3);
assert.deepEqual(loadedLegacy.birthday, { month: 6, day: 1 });
assert(loadedLegacy.claimedRewardIds.includes('mint_legacy_fixture_reward'));
assert.equal(loadedLegacy.partnerSchedule.schemaVersion, 5);
assert.equal(loadedLegacy.goldenAppleGacha.schemaVersion, 3);
assert.equal(loadedLegacy.classicEndgame.schemaVersion, 2);

const loadedDirectly = loadStoredPetJson(legacyRaw, fixtureNow);
assert.equal(loadedDirectly.name, loadedLegacy.name);
assert.equal(loadedDirectly.coins, loadedLegacy.coins);
